...
if is-extra-command-installed build ; then
  ...
  run_completion 4 i build fail '' | assert ii-client injixo sdk xlink
  run_completion 5 i build fail inj '' | assert ies.log security.log
  run_completion 5 i build last-log -t '' | assert ii-client injixo
  run_completion 6 i build last-log -t fr-ank '' | assert ies.log
  run_completion 6 i build last-log -t -s '' | assert-empty
  run_completion 7 i build last-log -t -s 10 '' | assert ii-client injixo
fi
